<?php
/**
 * Scheduler-Termine-Element.
 */

use KLXM\Scheduler\SchedulerRenderer;
use FriendsOfREDAXO\Builder\Starter\StarterConfig;

$schedulerAvailable = rex_addon::exists('scheduler')
    && rex_addon::get('scheduler')->isAvailable()
    && class_exists(SchedulerRenderer::class)
    && SchedulerRenderer::isAvailable();
...
foreach (SchedulerRenderer::getCategoryChoices() as $id => $name) {
    $catChoices[(string) $id] = $name . ' (#' . $id . ')';
}
foreach (SchedulerRenderer::getRepeatingEntryChoices() as $id => $name) {
...
foreach (SchedulerRenderer::getVenueChoices() as $id => $name) {
    $venueChoices[(string) $id] = $name . ' (#' . $id . ')';
}

return [
    'label' => 'Scheduler-Termine',
    'description' => 'Kommende Termine aus dem scheduler-Kalender (nach Kategorie oder Serientermin)',
    'version' => '1.15.0',
    'icon' => 'fa-calendar',
    'category' => 'data',
    ...
    'fields' => array_merge(
        ...
        [
            ...
            'limit' => [
                ...
                'notice' => '1-' . SchedulerRenderer::MAX_LIMIT . '. Default: 6.',
                'default' => '6',
            ],
            'show_image' => [
                'type' => 'checkbox',
            ...
            'image_field' => [
                'type' => 'text',
                'label' => 'Scheduler-Feldname fuer Bild',
                'notice' => 'Name des Media-Felds aus dem scheduler-Fieldset (z.B. <code>image</code>, <code>bild</code>, <code>header_image</code>). Leer = kein Bild.',
                'default' => '',
            ],
            'show_links' => [
                'type' => 'checkbox',
